 ...
 *
 */
public class CmdCopyHashedSecret extends CmdSwingAbstract<ICommandableAccount> {

   protected final PascalCmdManager pcm;

   protected String                 keyBase = "cmd.key.copy.hashedsecret";

   public CmdCopyHashedSecret(PascalCmdManager pcm) {
   ...
   public void executeWith(Account ac) {
      if (ac != null) {
         ...
         if (ac.getHashedSecret() != null) {
            psc.copyToClipboard(ac.getHashedSecret(), "Hashed Secret");
         } else {
            psc.getLog().consoleLogError("Hashed Secret is null or empty");
         }
      }
   }
